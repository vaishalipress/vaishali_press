...
const getMarketMonthlySalesReport = async (
    district: string,
    market: string,
    ...
    const pipeline: PipelineStage[] = [
        // Stage 1: Lookup client information to get district and market
        {
            $lookup: {
                from: "clients",
                localField: "client",
        ...
        // Stage 3: Filter sales by district, market, date range and optional product filter
        {
            $match: {
                "clientInfo.district": district,
                "clientInfo.market": market,
                date: dateMatch,
...
export const GET = async (req: Request) => {
    try {
        ...
        const monthlySales = await getMarketMonthlySalesReport(
            district,
            market,
            from,
            to,
        ...
        console.log("Error in Market monthly sales", error);
        return Response.json(
