  ...
  const searchClientByName = useCallback(
    (name: string) => {
      ...
      const searchedClient = [...clientsData].filter((client) => {
        if (sort === "district") {
          return client?.district.startsWith(name?.toLocaleLowerCase());
        } else {
      ...
      <div className="flex items-center justify-between gap-3 mb-3 px-3">
        ...
        <div className="flex gap-3">
          ...
          <Button
            variant={"ghost"}
            size={"sm"}
            onClick={() => downloadToPDF(isLoading, "#clients", `clients.pdf`)}
          >
        ...
        <Table id="clients">
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="uppercase min-w-[120px]">Month</TableHead>
              <TableHead className="uppercase min-w-[120px]">Date</TableHead>
              <TableHead className="uppercase min-w-[120px]">Mobile</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            ...
            {data?.map((client, idx) => (
              ...
              >
                ...
                <TableCell className="capitalize">
                  {client?.market
                    ? client?.market?.charAt(0)?.toUpperCase() +
                      client?.market?.substring(1)
                    : "NA"}
                </TableCell>

                <TableCell className="capitalize">
                  {client?.district?.charAt(0)?.toUpperCase() +
                    client?.district?.substring(1)}
                </TableCell>
                <TableCell>
                  {format(new Date(client?.createdAt), "MMMM")}
                </TableCell>
                <TableCell>
